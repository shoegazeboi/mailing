...
def whatsapp(number, text): #part_start: рассылка на вотс.
    ...
    options.add_argument('--user-data-dir={}\Google\Chrome\\User Data'.format(db[1]))
...
def telegram(numbers, text): #part_start: рассылка на телегу.
    ...
    for number in numbers: #Part_in_Part: Самое мясо. Сдесь произходит сама рассылка, нажимаются кнопочки и тд.
        ...
        sleep(time_to_sleep)
        driver_delay.until(EC.presence_of_element_located((By.XPATH, '//*[@id="ng-app"]/body/div[5]/div[2]/div/div/div[3]/div/button'))).click()

        sleep(time_to_sleep)
        send_keys_to('//*[@id="ng-app"]/body/div[6]/div[2]/div/div/div[1]/form/div[1]', number) # Вписывает номер в строку.
